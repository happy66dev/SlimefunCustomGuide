    ...
    private void handleMaterials(HttpExchange exchange) throws IOException {
        ...
        if (typeFilter.isEmpty() || typeFilter.equals("VANILLA")) {
            for (Material mat : Material.values()) {
                if (mat.isItem() && (mat.name().toLowerCase().contains(q) || ColorUtil.stripColorCodes(VanillaMaterialLocalization.getItemName(mat)).toLowerCase().contains(q))) {
                    if (!first) sb.append(',');
